...
def main():
    ...
    for key, value in embroideryFoldersDict.items():
        ...
        print(f"{Fore.RED}{key:<{width}} : {Fore.YELLOW}{str(embroideryFoldersDict[key]['location']):<80} : {validLocation}")
    print(f"{Fore.YELLOW}Excluded Folders: {', '.join(excluded_folders)}")
    print(f"{Fore.RED}Starting {Fore.YELLOW}{MAX_FILES:,}{Fore.RED} loops images from {Fore.YELLOW}{count_files_pathlib():,}{Fore.RED} VXX files")
    ...
    for dirpath, dirnames, filenames in os.walk(root_embroidery_directory, topdown=True):
        ...
        if DEBUG:
            print(f"{'Dirpath':<{width}}: {dirpath}")
            print(f"{'Dirnames':<{width}}: {dirnames}")
            print(f"{'Filenames':<{width}}: {filenames}")
        ...
        print(f"{'Filenames':<{width}}: {matching_files}")
        ...
        for index2, filename in enumerate(matching_files):
            ...
            if metaName == lastMetaName:
                print(f"{Fore.RED}Skipping {metaName} for {filename}")
                continue
            print(f"{Fore.GREEN}Processing {metaName} for {dirpath}/{filename}")
            ...
            if fnmatch.fnmatch(VXX_Filename, fPattern_VP3):
                ...
                if matching_JPG_files:
                    if DEBUG:
                        print(f"VXX_Filename {index2}: {VXX_Filename}")
                        print(f"JPG Images {index2}: {matching_JPG_files}")
                    Image_Filename  = f"{os.path.join(images_folder, CatalogName, matching_JPG_files[0])}"
                    if DEBUG: print(f"JPG Image Filename {index2}: {Image_Filename}")
                    Image_infile    = f"{os.path.join(dirpath,  matching_JPG_files[0])}"
                    if DEBUG: print(f"JPG Image infile {index2}: {Image_infile}")
                    copy_file(Image_infile,Image_Filename)
                else:
                    print(f"No JPG Images found: {dirpath}")
                    break
            if DEBUG:
                print(f"Catalog Name {index2}: {CatalogName}")
                print(f"metaName {index2}: {metaName}")
                print(f"VXX filename {index2}: {filename}")
                print(f"Image_filename {index2}: {Image_Filename}")
    ...
    if DEBUG: print(json.dumps(VXX_dictionary, indent=4))
    ...
    for CatalogName, inner_dict in  VXX_dictionary.items():
        ...
        if CatalogName not in catalogFoldersList:
            ...
            if not os.path.exists(CatalogFolder):
                if DEBUG: print(f"{Fore.YELLOW}Error: '{CatalogFolder}' does not exist. Creating...")
        ...
        for metaName, value in inner_dict.items():
            ...
            if DEBUG:
                print(f'{Fore.CYAN}MetaName', metaName)
                print('  VXX_Filename: ',VXX_Filename)
                ...
                print(f" Image: {Image_Filename} {ImageExistsStatus}")
            if not ImageExists:
                if fnmatch.fnmatch(VXX_Filename, fPattern_VP3):
    ...
    print(f"{Fore.RED}Image catalog generation complete.{Style.RESET_ALL}")
...
def getCatalogFolderName(dirpath):
    ...
    try:
        ...
        if DEBUG:
            print(f"Catalog Folder: {parentFolderName}")
    except ValueError:
        print(f"'Error: 'Embroidery Thumbnails' not found in the '{dirpath}'")
        exit(99)
...
def cdToHomeFolder():
    ...
    try:
        ...
        print(f"Error: The directory {downloads_folder} was not found.")
        exit(99)
    except Exception as e:
        print(f"An error occurred changing directory to {downloads_folder}: {e}")
        exit(99)

def makePic(infile,outfile):
    # Read the design
    if DEBUG: print(f"Reading : {infile}")
    try:
        pattern = pyembroidery.read(infile)
    except (TypeError, AttributeError) as e:
        print(f"An error occurred reading {infile}")
        print(f"Error: {e}")
#        removed_item = catalogList.pop()
#        print(f"Removed bad list item from CatalogList: {removed_item}")
        remove_file(infile)
        return

    # Write the image (visual representation)
    if DEBUG: print(f"Writing Image file to: {outfile}")
...
def ask_yes_no(question):
    ...
    while True:
        ...
        if reply in ("yes", "y"):
            ...
            print(f"{Fore.YELLOW}Sorry, invalid input. Please try again.")

def remove_file(infile):
    if not os.path.exists(infile):
        print(f"{Fore.RED}Error: File '{infile}' does not exist.")
        return

    if ask_yes_no(f"{Fore.RED}Are you sure you want to delete '{infile}'?"):
            try:
                send2trash(infile)
                print(f"{Fore.YELLOW}File '{infile}' moved to trash successfully.")
            except send2trash.TrashPermissionError:
                print(f"{Fore.RED}Could not move '{infile}' to trash due to a permission error.")
            except Exception as e:
                print(f"{Fore.RED}An unexpected error occurred: {e}")
    else:
        print("{Fore.RED}File Deletion cancelled.")
...
def set_finder_comment(file_path, comment_text):
    ...
    try:
        ...
        if DEBUG: print(f"Comment set for {file_path}")
    except Exception as e:
...
#   Create Table of Folder Links
    ...
    for CatalogName, inner_dict in VXX_dictionary.items():
        ...
        if not DEBUG: print(f"Processing {CatalogName}")
        for metaName, value in inner_dict.items():
            VXX_Filename = VXX_dictionary[CatalogName][metaName]['VXX_filename']
            Image_Filename = VXX_dictionary[CatalogName][metaName]['Image_filename']
            if DEBUG:
                print(f". metaName   : {metaName}")
                print(f". VXX_Filename: {VXX_Filename}")
...
def download_image_to_folder(image_url, folder_name, image_filename):
    ...
    if not os.path.exists(folder_name):
        print(f"Error: Folder '{folder_name}' does not exist for the favicon image.")
        exit(100)
